{
  ...
  "nodes": [
          ...
          "values": [
            ...
            },
            {
              "conditions": {
                ...
                "conditions": [
                  {
                    "id": "42905bc4-3625-49dc-8e38-7906e85f03fc",
                    "leftValue": "={{ $('Webhook').item.json.query.action }}",
                    "rightValue": "update-seats",
                    "operator": {
                      "type": "string",
                      "operation": "equals",
                      "name": "filter.operator.equals"
    ...
    },
    {
      "parameters": {
        ...
        "query": "INSERT INTO miomente_partner_portal_sessions (user_id, token, expires_at)\nVALUES ('{{ $json.id }}', '{{ $json.token }}', NOW() + INTERVAL '14 days');",
        "options": {}
      },
          ...
          "values": [
            ...
            },
            {
              "conditions": {
                ...
                "conditions": [
                  {
                    "id": "439a148e-4330-4cdd-afe9-2256f3649ac5",
                    "leftValue": "={{ $('Webhook').item.json.query.action }}",
                    "rightValue": "update-seats",
                    "operator": {
                      "type": "string",
    ...
    {
      "parameters": {
        "content": "## Окружение Odoo (stage/prod)",
        "height": 208,
        "width": 400,
        "color": 6
      },
    ...
    },
    {
      "parameters": {
        ...
        "sendBody": true,
        "specifyBody": "json",
        "jsonBody": "={\n  \"personalizations\": [{\n    \"to\": [{ \"email\": \"{{ $json.email }}\" }],\n    \"subject\": \"Passwort zurücksetzen - Miomente Partner-Portal\"\n  }],\n  \"from\": {\n    \"email\": \"bestellung@miomente.de\",\n    \"name\": \"Miomente Partner-Portal\"\n  },\n  \"content\": [{\n    \"type\": \"text/html\",\n    \"value\": \"<div style='background-color: #f9f9f9; padding: 40px 0; font-family: Helvetica, Arial, sans-serif;'> <table align='center' border='0' cellpadding='0' cellspacing='0' width='600' style='background-color: #ffffff; border-radius: 8px; overflow: hidden; border: 1px solid #eeeeee;'> <tr> <td align='center' style='padding: 30px 0; background-color: #ffffff;'> <img src='https://www.miomente.de/skin/frontend/ultimo/default/images/goldenwebage/logo.png' alt='Miomente' width='180' style='display: block;'> </td> </tr> <tr> <td style='padding: 40px 30px;'> <h2 style='color: #333333; font-size: 24px; margin-top: 0;'>Passwort zurücksetzen</h2> <p style='color: #666666; font-size: 16px; line-height: 1.5;'>Hallo {{ $json.name }},</p> <p style='color: #666666; font-size: 16px; line-height: 1.5;'>wir haben eine Anfrage zum Zurücksetzen Ihres Passworts erhalten. Klicken Sie auf den untenstehenden Button, um ein neues Passwort festzulegen:</p> <div style='padding: 30px 0; text-align: center;'> <a href=\\\"{{ $('CONSTANTS1').item.json.APP.domain }}/reset-password?token={{ $json.reset_token }}\\\" style='background-color: #e4002b; color: #ffffff; padding: 15px 30px; text-decoration: none; border-radius: 4px; font-weight: bold; display: inline-block;'>Passwort jetzt zurücksetzen</a> </div> <p style='color: #999999; font-size: 14px; line-height: 1.5;'>Dieser Link ist für <strong>1 Stunde</strong> gültig. Falls Sie dies nicht angefragt haben, können Sie diese E-Mail einfach ignorieren.</p> </td> </tr> <tr> <td style='padding: 20px 30px; background-color: #f1f1f1; color: #888888; font-size: 12px; text-align: center;'> <p style='margin: 0;'>Herzliche Grüße,<br><strong>Ihr Miomente-Team</strong></p> <p style='margin-top: 15px;'>Boni-Shop GmbH | <a href='https://www.miomente.de' style='color: #888888;'>www.miomente.de</a></p> </td> </tr> </table> </div>\"\n  }],\n  \"tracking_settings\": {\n    \"click_tracking\": {\n      \"enable\": false,\n      \"enable_text\": false\n    },\n    \"open_tracking\": {\n      \"enable\": false\n    }\n  }\n}",
        "options": {}
      },
    ...
    },
    {
      "parameters": {
        "respondWith": "json",
        "responseBody": "={{ { \"success\": true, \"message\": \"Seats updated successfully\" } }}",
        "options": {}
      },
      "id": "193ba793-5c36-45bc-961b-4e498aeb7ae3",
      "name": "Update Date Response1",
      "type": "n8n-nodes-base.respondToWebhook",
      "typeVersion": 1.1,
      "position": [
        -2896,
    ...
    {
      "parameters": {
        "content": "## UPDATE COURSE SEATS (DONE)",
        "height": 256,
        "width": 1360,
        "color": 4
      },
      ...
      "id": "11551845-4b54-430e-b868-3f6ec203da50",
      "name": "Sticky Note26"
    },
    {
      "parameters": {
        "operation": "executeQuery",
        "query": "-- 1. Update wishdatefinder (frontend listing)\nUPDATE miomente_wishdatefinder_index \nSET simple_product_seats = {{ $('Webhook').item.json.body.seats }},\n    updated = NOW()\nWHERE simple_product_id = {{ $('Webhook').item.json.body.date_id }};\n\n-- 2. Update simple product (booking capacity)\nUPDATE catalog_product_entity_varchar \nSET value = '{{ $('Webhook').item.json.body.seats }}'\nWHERE entity_id = {{ $('Webhook').item.json.body.date_id }}\n  AND attribute_id = 720\n  AND store_id = 0;\n\n-- 3. Update parent product (display on page)\nUPDATE catalog_product_entity_varchar \nSET value = '{{ $('Webhook').item.json.body.seats }}'\nWHERE entity_id = (\n    SELECT parent_id FROM catalog_product_super_link \n    WHERE product_id = {{ $('Webhook').item.json.body.date_id }}\n)\n  AND attribute_id = 720\n  AND store_id = 0;\n\n-- 4. Update stock item qty\nUPDATE cataloginventory_stock_item \nSET qty = {{ $('Webhook').item.json.body.seats }}\nWHERE product_id = {{ $('Webhook').item.json.body.date_id }};\n\n-- 5. Update stock status qty\nUPDATE cataloginventory_stock_status \nSET qty = {{ $('Webhook').item.json.body.seats }}\nWHERE product_id = {{ $('Webhook').item.json.body.date_id }};\n\n-- Return confirmation\nSELECT \n    {{ $('Webhook').item.json.body.date_id }} AS date_id,\n    '{{ $('Webhook').item.json.body.seats }}' AS new_seats;",
        "options": {}
      },
      "id": "66956791-0425-4e28-be48-1c2974094b06",
      "name": "Update Seats",
      "type": "n8n-nodes-base.mySql",
      "typeVersion": 2.4,
      "position": [
    ...
    "Webhook": [
      {
        "json": {
          "headers": {
            "host": "n8n.boni.tools",
            "x-real-ip": "172.71.164.119",
            "x-real-port": "12161",
            "x-forwarded-for": "188.245.103.53, 172.71.164.119",
            "remote-host": "172.71.164.119",
            "connection": "upgrade",
            "content-length": "30",
            ...
            "cf-ray": "9bd5b6fd1b3fd2a0-FRA",
            "authorization": "Bearer info-1768314780635-6obobxwlry3",
            "cdn-loop": "cloudflare; loops=1",
            "cf-connecting-ip": "188.245.103.53",
            "cf-ipcountry": "DE",
  ...
  "versionId": "7e9998d3-33f1-4dd3-8dce-baf4641dbdf9",
  "meta": {
