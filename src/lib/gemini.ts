...
// ============================================
// Ticket Name Search Filter (Simple Text Search)
// ============================================

/**
 * Ticket data for filtering
 */
...
/**
 * Filter tickets by searching in ticket name
 * Simple case-insensitive text search - no AI involved
 *
 * @param tickets - Array of tickets with IDs and names
 * @param query - Search query to match against ticket names
 * @returns Matching ticket IDs
 */
export function filterTicketsByName(
  tickets: TicketFilterData[],
  query: string
): FilterResult {
  // Empty query returns all tickets
  if (!query.trim()) {
    return {
  ...
  const matchingIds = tickets
    .filter(ticket => {
      const nameLower = ticket.ticketName.toLowerCase();
...
export interface TicketFilterSummary {
  ticketId: number;
  ticketName: string;
  // Keep old fields as optional for gradual migration
  category?: AICategory;
  urgency?: AIUrgency;
...
/**
 * @deprecated Use filterTicketsByName instead
 * Legacy function for backward compatibility - now just does simple name search
 */
export function filterTicketsByQuery(
  ...
  const ticketData: TicketFilterData[] = tickets.map(t => ({
    ticketId: t.ticketId,
    ticketName: t.ticketName || t.summary || '',
  }));

  const result = filterTicketsByName(ticketData, query);

  return {
    matchingIds: result.matchingIds,
    interpretation: query ? `Search: "${query}"` : '',
