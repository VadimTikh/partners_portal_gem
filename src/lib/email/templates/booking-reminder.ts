 ...
 * Sent to partner when booking hasn't been confirmed after 24h/48h
 */
...
export interface BookingReminderData {
  ...
  declineUrl: string;
  portalUrl: string;
  reminderNumber: 1 | 2; // 1 = first reminder (24h), 2 = second reminder (48h)
  hoursWaiting: number;
}
 ...
 */
export function getBookingReminderHtml(data: BookingReminderData): string {
  const urgencyColor = data.reminderNumber === 2 ? '#dc3545' : '#ffc107';
  const urgencyBgColor = data.reminderNumber === 2 ? '#fef2f2' : '#fff8e6';
  const urgencyText = data.reminderNumber === 2
    ? 'Dringend: Letzte Erinnerung vor Eskalation'
    : 'Erinnerung: Buchung wartet auf Bestätigung';
...
<div style='background-color: #f9f9f9; padding: 40px 0; font-family: Helvetica, Arial, sans-serif;'>
  <table align='center' border='0' cellpadding='0' cellspacing='0' width='600' style='background-color: #ffffff; border-radius: 8px; overflow: hidden; border: 1px solid #eeeeee;'>
    ...
    <!-- Urgency Banner -->
    <tr>
      <td style='padding: 15px 30px; background-color: ${urgencyBgColor}; border-left: 4px solid ${urgencyColor};'>
    ...
    <tr>
      <td style='padding: 40px 30px;'>
        <h2 style='color: #333333; font-size: 24px; margin-top: 0;'>
          ${data.reminderNumber === 2 ? 'Letzte Erinnerung: Buchung bestätigen' : 'Erinnerung: Offene Buchung'}
        </h2>
        <p style='color: #666666; font-size: 16px; line-height: 1.5;'>Hallo ${escapeHtml(data.partnerName)},</p>

        ${data.reminderNumber === 2 ? `
        <p style='color: #dc3545; font-size: 16px; line-height: 1.5; font-weight: bold;'>
          Eine Buchung wartet seit über ${data.hoursWaiting} Stunden auf Ihre Bestätigung. Bitte reagieren Sie umgehend, da sonst ein Support-Ticket erstellt wird.
        </p>
        ` : `
        <p style='color: #666666; font-size: 16px; line-height: 1.5;'>
          Eine Buchung wartet seit ${data.hoursWaiting} Stunden auf Ihre Bestätigung. Bitte bestätigen oder lehnen Sie die Buchung zeitnah ab:
        </p>
        `}
        ...
        <div style='padding: 30px 0; text-align: center;'>
          <a href="${data.confirmUrl}" style='background-color: #28a745; color: #ffffff; padding: 15px 40px; text-decoration: none; border-radius: 4px; font-weight: bold; display: inline-block; margin: 5px;'>Jetzt bestätigen</a>
          <br><br>
        ...
        ${data.reminderNumber === 2 ? `
        <p style='color: #dc3545; font-size: 14px; line-height: 1.5; font-weight: bold;'>
          Hinweis: Falls keine Reaktion erfolgt, wird unser Support-Team die Buchung telefonisch mit Ihnen klären.
        </p>
        ` : ''}
      </td>
    </tr>
...
/**
 * Generate booking reminder email subject
 */
export function getBookingReminderSubject(
  ...
  if (reminderNumber === 2) {
    return `DRINGEND: ${courseName} am ${eventDate} - Letzte Erinnerung`;
  }
  return `Erinnerung: ${courseName} am ${eventDate} - Bestätigung erforderlich`;
}

/**
 * Generate plain text version of the email
 */
export function getBookingReminderText(data: BookingReminderData): string {
  const urgencyText = data.reminderNumber === 2
    ? `DRINGEND: Eine Buchung wartet seit über ${data.hoursWaiting} Stunden auf Ihre Bestätigung. Bitte reagieren Sie umgehend, da sonst ein Support-Ticket erstellt wird.`
    : `ERINNERUNG: Eine Buchung wartet seit ${data.hoursWaiting} Stunden auf Ihre Bestätigung.`;
...
AKTIONEN
Jetzt bestätigen: ${data.confirmUrl}
Buchung ablehnen: ${data.declineUrl}

Sie können die Buchung auch im Partner-Portal verwalten: ${data.portalUrl}

${data.reminderNumber === 2 ? 'HINWEIS: Falls keine Reaktion erfolgt, wird unser Support-Team die Buchung telefonisch mit Ihnen klären.' : ''}
